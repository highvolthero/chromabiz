...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all chromabiz.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*chromabiz\.com$/)) {
        return true;
      }

      // Allow all chromabizai.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*chromabizai\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(`git -c user.name="visual-edit" -c user.email="support@chromabiz.com" add "${result.file}"`);
              execSync(`git -c user.name="visual-edit" -c user.email="support@chromabiz.com" commit -m "visual_edit_variable_${timestamp}"`);
            } catch (gitError) {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@chromabiz.com" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@chromabiz.com" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
